...
**AI-scored job matching for people doing an active job search — not endless scrolling.**

Upload a resume once. JobMatch pulls real postings from free job-board APIs (India +
worldwide), scores your fit against every one of them with Gemini, and emails you your
best matches — all running on free-tier infrastructure.
...
[![Tests](https://img.shields.io/badge/tests-Jest%20%2B%20Vitest-C21325?logo=jest&logoColor=white)](#9-testing)
...
Job boards are a firehose. You re-read the same posting five times trying to guess whether
you're actually qualified. JobMatch flips that: it scores *you against the posting*, ranks
what's worth your time, and gets out of the way.

It's also a portfolio piece — a MERN stack built with real security and operational
constraints taken seriously: **IDOR-safe data access**, **API-quota budgeting**, and
**dedupe-safe background jobs**, not just CRUD-with-a-login-page.
...
2. [Prerequisites — free API keys](#2-prerequisites--register-for-these-free-api-keys-first)
3. [Backend setup](#3-backend-setup)
4. [Frontend setup](#4-frontend-setup)
5. [IDOR / ownership security model](#5-idor--ownership-security-model)
6. [Design: the "signal scanner"](#6-design-the-signal-scanner)
7. [Job ingestion: rotation budget & bootstrap sync](#7-job-ingestion-rotation-budget-and-bootstrap-sync)
8. [Email digests](#8-email-digests-two-mechanisms-one-shared-dedupe-flag)
9. [Testing](#9-testing)
...
> **Request flow, always**: `route → service → repository → Mongoose`. Routes never import
> a Mongoose model directly. This isn't just style — it's what makes the ownership guarantee
> in [section 5](#5-idor--ownership-security-model) possible to state and test as *one rule*
> instead of re-litigating it per route.
...
|---|---|---|
| **Adzuna** | Job postings (free tier: 250 calls/day, 25/min) | [developer.adzuna.com](https://developer.adzuna.com/) |
| **Jooble** | Job postings (free tier, no published cap) | [jooble.org/api/about](https://jooble.org/api/about) |
| **Google Cloud Console** | OAuth client ID for "Sign in with Google" | [console.cloud.google.com](https://console.cloud.google.com/apis/credentials) |
| **Google AI Studio** | Gemini API key for match scoring | [aistudio.google.com](https://aistudio.google.com/app/apikey) |
| **Brevo** *(recommended)* | Free SMTP relay (300 emails/day) for digests | [app.brevo.com](https://app.brevo.com/settings/keys/smtp) |
| **Gmail SMTP** *(alternative)* | Low-volume SMTP if you'd rather skip Brevo | `smtp.gmail.com:587` with a Google App Password |

`Arbeitnow` needs no key at all — it's already good to go.
...
> This is a one-time command (see [section 7](#7-job-ingestion-rotation-budget-and-bootstrap-sync))
> — not something to add to a cron schedule yourself. The app already schedules ongoing
...
`userId` baked **directly into the Mongo query filter**, never fetched first and checked
after:
...
(`components/ScoreGauge.jsx`), not a progress bar, color-banded 🟢 green (≥75) / 🟡 amber
(50–74) / 🔴 red (<50). Data — scores, timestamps, source labels — is set in **JetBrains
Mono** so it reads as an instrument readout, while headings use **Space Grotesk** and body
copy uses **Inter**. Tokens live in `frontend/src/styles/tokens.css`.
...
> ⚠️ If you shorten the cron interval or raise `ADZUNA_CALLS_PER_RUN`, re-check this math
> against the 250/day cap before deploying.
...
ingestion source has reconfirmed within `JOB_STALE_DAYS` (default 21) is set
`isActive: false` — never deleted — so a stale role stops appearing without losing its
history. Manually-created jobs (`source: 'manual'`) are never auto-deactivated.
...
[section 5](#5-idor--ownership-security-model), match scoring (clamping + upsert-not-duplicate),
digest logic (score threshold, opt-out, dedupe), ingestion rotation (cursor
advance/wrap/persistence per source), and stale-job deactivation.
...
**☁️ Managed free hosts** *(recommended for a real deployment)*

| Layer | Suggested host | Notes |
|---|---|---|
| Backend | Render or Fly.io (Node web service) | Set every variable from `backend/.env.example` in the host's secrets UI. Free tiers that spin down on idle will **pause the schedulers** — pick a plan that stays alive, or trigger cron endpoints externally (e.g. cron-job.org hitting a small internal trigger route) as a workaround. |
| Frontend | Vercel or Netlify | Set `VITE_API_BASE_URL` to your deployed backend's URL and `VITE_GOOGLE_CLIENT_ID` to match the backend. |
